...
def _msys_path_to_windows(path: str) -> str:
    """Convert a Git Bash/MSYS path like /e/work to E:\\work."""
    if not (_IS_WINDOWS and path):
        return path
    normalized = path.replace("\\", "/")
    if len(normalized) >= 3 and normalized[0] == "/" and normalized[1].isalpha() and normalized[2] == "/":
        return f"{normalized[1].upper()}:\\" + normalized[3:].replace("/", "\\")
    return path
...
class LocalEnvironment(BaseEnvironment):
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        busybox = os.environ.get("HERMES_BUSYBOX_PATH", "") if _IS_WINDOWS else ""
    ...
    def _update_cwd(self, result: dict):
        ...
        try:
            with open(self._cwd_file) as f:
                cwd_path = _msys_path_to_windows(f.read().strip())
            if cwd_path:
                self.cwd = cwd_path
        ...
        self.cwd = _msys_path_to_windows(self.cwd)
