...
:: Try 'python3' command (Git Bash / MSYS2)
...
if defined PYTHON_CMD (
    python3 -c "import sys; raise SystemExit(0 if sys.version_info[:2] <= (3,12) else 1)" >nul 2>&1 || set "PYTHON_UNTESTED=1"
    exit /b 0
)
:: Try bare 'py' as fallback
py --version >nul 2>&1 && py -c "import sys; raise SystemExit(0 if sys.version_info >= (3,11) else 1)" >nul 2>&1 && set "PYTHON_CMD=py"
if defined PYTHON_CMD (
    py -c "import sys; raise SystemExit(0 if sys.version_info[:2] <= (3,12) else 1)" >nul 2>&1 || set "PYTHON_UNTESTED=1"
)
exit /b 0
